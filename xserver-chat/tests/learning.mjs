...
fs.writeFileSync(app+'/seed.php',`<?php require __DIR__.'/bootstrap.php'; $id=(int)$_GET['id']; query("INSERT OR IGNORE INTO users(id,login,name,password,role,created_at) VALUES(?,?,?,'unused','member',0)",[$id,'test'.$id.'@example.com','テスト'.$id]);$u=query('SELECT * FROM users WHERE id=?',[$id])->fetch();signIn($u);output(['csrf'=>$_SESSION['csrf']]);`);
...
try{
for(const id of [1,2]){const r=await run(id,'seed.php?id='+id);assert.equal(r.httpStatusCode,200,r.text+r.errors);jars[id].csrf=JSON.parse(r.text).csrf;}
...
console.log('PASS: anonymous protection, per-user isolation, persistence after login, CSRF, stale-edit conflict, escaped memo, validation, uncheck and clearing.');
