...
?>
<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>議事録・お知らせ｜じねんネットワーク</title><link rel="stylesheet" href="operations.css"><link rel="stylesheet" href="site-nav.css?v=20260913-menu"></head><body><header><a href="./">じねんネットワーク</a><span>担当者のページ</span></header><nav><?php if($ready):?><a href="mail-center.php">メールでお知らせ</a><?php endif;?><a href="./">チャットへ戻る</a><?php if($board):?><a href="operations.php">議事録</a><?php endif;?><?php if($notice):?><a href="?tab=notice">お知らせ投稿</a><?php endif;?><?php if($admin):?><a href="?tab=editors">地区担当者の設定</a><?php endif;?></nav><main>
...
<?php elseif($item):?><p><a href="operations.php">← 議事録一覧</a></p><?php if(isset($_GET['saved'])):?><p role="status">保存しました。内容をご確認ください。</p><?php endif;?><article class="card"><p><?= (int)$item['published']?'公開済み':'下書き' ?> ／ 開催日 <?= $e($item['meeting_on']) ?></p><h1><?= $e($item['title']) ?></h1><div class="body-text"><?= $e($item['body']) ?></div><?php if($item['resource_url']):?><p><a class="button" href="<?= $e($item['resource_url']) ?>">関連資料を開く</a></p><?php endif;?><p><a class="button" href="?edit=1&amp;id=<?= (int)$item['id'] ?>">編集する</a></p><p><a class="button secondary" href="?copy=1&amp;id=<?= (int)$item['id'] ?>">この議事録をコピーして新規作成</a></p><?php if(!(int)$item['published']):?><form method="post"><input type="hidden" name="action" value="publish_minutes"><input type="hidden" name="csrf" value="<?= $e($_SESSION['csrf']) ?>"><input type="hidden" name="id" value="<?= (int)$item['id'] ?>"><input type="hidden" name="revision" value="<?= (int)$item['revision'] ?>"><p><a class="button" href="mail-center.php?minutes=<?= (int)$item['id'] ?>">公開してメールでも知らせる →</a></p><p>メールを送らずサイトだけに公開する場合：</p><button>この内容で公開する</button></form><?php endif;?><?php if((int)$item['published']):?><p><a class="button" href="mail-center.php?minutes=<?= (int)$item['id'] ?>">この議事録をメールで知らせる</a></p><?php endif;?><small>最終更新：<?= $e(date('Y/m/d H:i',(int)$item['updated_at'])) ?></small></article>
...
</main><script src="site-nav.js?v=20260913-menu" defer></script></body></html>
