...
function clearPrivate(){requestGeneration++;user=null;['chat-panel','account','admin-panel','navigation','next-zoom'].forEach(id=>$(id).hidden=true);['messages','events','users','identity','next-zoom-date','next-zoom-title','applications'].forEach(id=>$(id).replaceChildren());$('email-form').reset();$('password-form').reset();postForm.reset();clearReply();$('credential').textContent='';$('credential-user').textContent='';credentialText='';if($('credential-dialog').open)$('credential-dialog').close();}
