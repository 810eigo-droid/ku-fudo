...
function clearPrivate(){requestGeneration++;user=null;['chat-panel','account','admin-panel','navigation'].forEach(id=>$(id).hidden=true);['messages','events','users','identity'].forEach(id=>$(id).replaceChildren());$('email-form').reset();$('password-form').reset();postForm.reset();clearReply();$('credential').textContent='';$('credential-user').textContent='';if($('credential-dialog').open)$('credential-dialog').close();}
...
async function start(){
 ...
 const required=Number(user.must_change)===1;$('password-required').hidden=!required;$('password-details').open=required;$('email-details').hidden=required;
...
bindForm('user-form',async form=>{const data=formData(form);if(data.role==='admin'&&!confirm('管理者は会員管理と両方のチャット操作ができます。追加しますか？'))return;const result=await api('user_create',data);showCredential('メールアドレス：'+data.login,result.temporary_password);form.reset();await loadUsers();});
