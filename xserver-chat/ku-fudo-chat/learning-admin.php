...
header("Content-Security-Policy: default-src 'self'; script-src 'self'; style-src 'self'; object-src 'none'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'");
?>
<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>会員の視聴状況｜じねんネットワーク</title><link rel="stylesheet" href="learning.css"><link rel="stylesheet" href="site-nav.css?v=20260913-menu"></head><body><header><a href="./?account=1">じねんネットワーク<span>管理者用</span></a></header><nav><a href="./?account=1">会員管理へ戻る</a><a href="learning.php">自分のマイページ</a></nav><main><h1>会員の視聴状況</h1><p>カリキュラム · 全<?= count($lessons) ?>レッスン</p><p>ご本人がチェックして保存した視聴状況です。動画の再生時間を自動計測した記録ではありません。</p><form method="get" action="learning-admin.php"><label for="search">お名前で探す</label><input id="search" name="q" maxlength="60" value="<?= $escape($search) ?>"><button class="primary">検索する</button></form><p><?= $total ?>人中 <?= count($members) ?>人を表示</p>
...
<section class="card"><h2><?= $escape($member['name']) ?>さん <?= (int)$member['active']?'':'（利用停止中）' ?></h2><p><strong><?= $done ?> / <?= count($lessons) ?> レッスン 視聴済み</strong></p><ul>
<?php foreach($lessons as $i=>$l): $r=$saved[$l['id']] ?? null; ?><li><strong><?= (int)($r['watched'] ?? 0)?'✓ 視聴済み':'未チェック' ?></strong> · <?= $escape($l['course']) ?> · <?= $escape($l['title']) ?></li><?php endforeach; ?>
</ul></section><?php endforeach; ?>
<div><?php if ($page>1): ?><a class="secondary" href="?q=<?= rawurlencode($search) ?>&amp;page=<?= $page-1 ?>">前の50人</a><?php endif; ?> <?php if ($page*50<$total): ?><a class="secondary" href="?q=<?= rawurlencode($search) ?>&amp;page=<?= $page+1 ?>">次の50人</a><?php endif; ?></div></main><script src="site-nav.js?v=20260913-menu" defer></script></body></html>
